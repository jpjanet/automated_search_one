...
\textit{val}&: \textrm{QUEUE\_ID} 
 						 \end{flalign*}};
 \node [longblock] (ljd) at (8,0) {submitted\_job\_dictionary \\
 \begin{flalign*}
\text{key}&: \textrm{JOB\_INFILE\_PATH} \\
\textit{val}&: \textrm{NUM\_OF\_SUBMITS} 
 						 \end{flalign*}}; 						 
\node [longblock] (ljd) at (16,0) {converged\_job\_dictionary \\
 \begin{flalign*}
\text{key}&: \textrm{JOB\_INFILE\_PATH} \\
...
%\path[line] (job_dispatcher) -- node[above,fontscale=0.25,label={[label distance=0.25cm]90:{job}}] {} (SGE);
%\path[line] ([yshift = - 0.25cm]state.west) -| node[near start,text width=1cm,text centered] {read state} ([xshift = -0.5cm]GA.south);
%\path[line] ([xshift = 0.5cm]GA.south) |- node[near start,text width=1cm,text centered] {write state} ([yshift =0.25cm]state.west);
